...
**Versión del Documento:** 3.11 (Sección 3.1 nueva: taxonomía completa de categorías de edificio — residencial, investigación, industrial, producción, militar, recolección, defensivo — y su regla de zona permitida (Núcleo A/Núcleo B/periferia/zona de influencia), más una nota de diseño futuro sobre mejoramiento de edificios por etapas. Diseño de base para la PoC de zonificación, ver `PoC_4/`)  
...
| **Fase 2 (En progreso — PoC 4: autoload de Ciudad/Avatar, HUD básico, y vínculo parcial con Player.gd completos): Ciudad y Demografía Visibles** | Integración de `Ciudad`/`Avatar` como autoload de Godot con HUD (comida, moral, nivel urbano); zonificación pintada sobre el grid con bloqueo de construcción fuera de zona; colonos como NPCs simples sin pathfinding avanzado. Dividida en 4 sub-proyectos independientes (ver `PoC_4/`): (1) autoload `Ciudad`/`Avatar` — **completo**; (2) HUD básico — **completo**; (3) zonificación; (4) NPCs colonos. Un sub-proyecto adicional, no planeado originalmente, vincula parcialmente `Ciudad` con `Player.gd` — **completo parcialmente**. | Medio | Mismos bloques con textura + UI simple + modelo low-poly genérico de colono | Estado de la ciudad (recursos, moral, nivel) visible y consistente con la lógica de Fase 0; zonificación aplicada en tiempo real. **Verificado (PoC 4):** `Ciudad.gd` (autoload registrado en `godot/project.godot`) reproduce los 6 tests de PoC 1 con los mismos valores, más `tasa_neta`/`recurso_critico` por recurso y `registrar_edificio_residencial()` (7 tests en total); `HUDLayer` en `Main.tscn` muestra nivel, población (ciudadanos/camas construidas, en rojo si excede la capacidad), moral, comida y recurso crítico en tiempo real (2 líneas cantidad/tasa con color blanco/rojo para comida y recurso crítico); `Player.gd` ahora llama a `Ciudad.suceder_avatar()` (tecla de prueba `K`, sin salud/combate real todavía) y a `Ciudad.registrar_edificio_residencial()` al declarar un edificio residencial válido (suma camas, no instalaciones — las casas construibles hoy no son fábricas de producción). Producción real de recursos y `instalaciones`/`fuentes_comida_activas` desde edificios reales siguen pendientes — ver Próximos Pasos de `PoC_4/`. |
