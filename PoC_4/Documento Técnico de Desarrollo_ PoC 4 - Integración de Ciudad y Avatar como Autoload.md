...
**✅ Verificado con Godot 4.7 (Steam) vía MCP:** `scenes/CiudadTest.tscn` corre y los 7 tests de `CiudadTest.gd` (puerto directo de los 6 asserts de PoC 1, 4 asserts de `tasa_neta`/`recurso_critico` para el HUD, y 1 test nuevo de `registrar_edificio_residencial`) pasan sin errores de `assert()`, con los mismos valores numéricos que la versión Python. `scenes/Main.tscn` sigue cargando sin errores nuevos (solo las advertencias `class_name` ya conocidas, ver PoC_3) con el autoload `Ciudad`, el HUD (`HUDLayer`) y los cambios en `Player.gd`.

Esta PoC cubre ya **3 de los 4 sub-proyectos** de la Fase 2: (1) autoload `Ciudad`/`Avatar`, (2) HUD básico, y (3) vínculo parcial de `Player.gd` con `Ciudad` (muerte de prueba + registro de camas construidas). Zonificación, producción real de recursos, y NPCs colonos quedan para sub-proyectos posteriores (ver Próximos Pasos).
...
Esta PoC cubre los tres primeros sub-proyectos de la **Fase 2 del roadmap** ("Ciudad y Demografía Visibles", GDD Sección 11), que originalmente empaquetaba 4 piezas independientes: el autoload de Ciudad/Avatar, un HUD básico, zonificación con bloqueo de construcción, y NPCs colonos. Se decidió diseñar e implementar cada pieza por separado.
...
* **Zonificación, NPCs colonos:** las otras 2 piezas de la Fase 2 original — cada una es un sub-proyecto independiente posterior.
...
godot/                      # Proyecto Godot compartido (ver GDD Sección 11)
  project.godot              # [autoload] Ciudad="*res://scripts/Ciudad.gd"
  scenes/
    CiudadTest.tscn           # Escena de pruebas (sin gameplay)
    Main.tscn                  # Ahora incluye HUDLayer (CanvasLayer + HUD.gd)
  scripts/
    ...
    Player.gd                  # (de PoC_3) Ahora llama a Ciudad.suceder_avatar() y Ciudad.registrar_edificio_residencial()
```
...
> 6. **Zonificación pintada sobre el grid** con bloqueo de construcción fuera de zona (depende de que `Ciudad` ya tenga un nivel real que consultar).
