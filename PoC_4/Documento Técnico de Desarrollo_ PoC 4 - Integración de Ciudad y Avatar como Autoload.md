...
* **Cámara cenital + overlay visual:** toggle (tecla `C`) entre 1ª persona y una cámara con perspectiva oblicua (centrada en el jugador, con paneo `WASD` y rotación orbital `Q`/`E` — ampliada a petición del usuario tras la primera prueba en vivo, ver 2.6); pintado de zonas por rectángulo de 2 clics; overlay traslúcido visible solo desde la cenital.
...
* **Cámara cenital completa (zoom, transición suave, selección de tropas por arrastre):** la versión implementada aquí ya tiene paneo y rotación orbital (agregados a petición del usuario, ver 2.6), pero sigue sin zoom, sin transición suave entre cámaras, y sin selección de tropas por arrastre. Eso es PoC 6 (Fase 4, ver GDD Sección 11 v3.14 — se corrió de Fase 3 a Fase 4 al insertarse "Mundo Procedural y Puestos de Recolección" antes).
* **Emplazar blueprints desde la vista cenital:** el GDD real exige que el bloqueo de zona también aplique al colocar edificios completos desde arriba, pero esa mecánica no existe todavía (depende de PoC 6 completo) — el bloqueo de esta PoC solo se aplica al declarar edificio en 1ª persona (`Player.gd::_declarar_edificio`). Ver 2.6.
...
godot/                      # Proyecto Godot compartido (ver GDD Sección 11)
  ...
  scripts/
    ...
    CamaraCenital.gd            # Cámara con perspectiva oblicua: toggle, paneo, órbita, pintado de zonas por 2 clics
...
**Cámara cenital (`CamaraCenital.gd`, script de un `Camera3D` en `Main.tscn`):** proyección con perspectiva (`PROJECTION_PERSPECTIVE`, `fov = 60`) en órbita clásica alrededor de un punto de mira (`foco`, sobre el plano `y = 0`) — nunca se mueve la cámara directamente, se recalcula su posición/orientación (`_actualizar_transform()`) a partir de `foco`, un ángulo de órbita (`angulo_orbital`) y una distancia/inclinación fijas (`DISTANCIA_CAMARA = 25.0`, `ANGULO_INCLINACION = 55°`). `posicionar_sobre(foco_xz)` centra el punto de mira sobre la posición real del jugador al activar el toggle (`Main.gd` le pasa `jugador.position.x/z`) — no sobre la zona de influencia ni el origen, para que la cámara siempre aparezca viendo donde está el jugador. Mientras la cenital está activa, `WASD` desplaza el punto de mira (paneo relativo a la orientación actual de la cámara) y `Q`/`E` giran la órbita alrededor de él — ambas teclas reutilizan el input de movimiento del jugador, que queda congelado en 1ª persona mientras tanto (sin conflicto). Pintado por rectángulo de 2 clics (estilo SimCity): teclas `1`/`2` seleccionan `ZONAS_PINTABLES[0]`/`[1]`; el primer clic marca una esquina (convertida de posición de pantalla a celda de grid vía raycast contra el plano `y = 0` y `VoxelWorld.local_to_map` — esta conversión funciona igual sin importar el ángulo/proyección de la cámara), el segundo pinta el rectángulo con `Zonificacion.pintar_zona()`. **Alcance reducido frente a PoC 6:** sin zoom, sin transición suave de cámara, sin selección de tropas por arrastre — la cámara cenital completa con esas piezas es Fase 4 del roadmap (GDD Sección 11 v3.14).

**Corrección tras la primera prueba en vivo (post-fusión):** la versión original de esta cámara era ortogonal recta (`PROJECTION_ORTHOGONAL`, mirando derecho hacia abajo, sin paneo) y se centraba en la zona de influencia (u origen si no había núcleo declarado) — al probarla jugando, el usuario reportó que necesitaba recorrer el mundo para ubicar la zona de influencia y prefería una perspectiva oblicua orbitable, más cercana a una cámara RTS real, en vez de una vista de planta pura. Se reemplazó por el diseño de órbita descrito arriba.
...
> 6. ~~Zonificación pintada sobre el grid con bloqueo de construcción fuera de zona.~~ **Completado (sub-proyecto 4):** autoload `Zonificacion.gd`, bootstrap del núcleo urbano, cámara cenital mínima, overlay visual y bloqueo real en `Player.gd::_declarar_edificio()` vía `validar_colocacion()` — ver 2.6. **Pendiente:** confirmar visualmente el toggle de cámara, el pintado por clic y la visibilidad condicionada del overlay jugando en el editor real (ver 3.5); cámara cenital completa (paneo/zoom, selección de tropas por arrastre — PoC 6, Fase 4); emplazar blueprints desde la vista cenital (depende de PoC 6 completo); los demás tipos de edificio de la taxonomía (GDD Sección 3.1: investigación, industrial, producción, militar, recolección, defensivo) — solo existe el tipo residencial construible hoy. **Detalles menores señalados por la revisión final (no bloquean, quedan para cuando se retome esta pieza):** `ZonaOverlay.reconstruir()` usa `queue_free()` antes de agregar los planos nuevos, lo que puede causar un frame de z-fighting entre planos viejos y nuevos si se llama dos veces muy seguido; no hay forma de cancelar la primera esquina de un rectángulo ya iniciado (un misclick fuerza a pintar); la cámara cenital usa un `size` ortogonal fijo (`40.0`) que puede no encuadrar una zona de influencia más grande que la mínima; el `VBoxContainer` del HUD no tiene `mouse_filter` configurado, así que podría absorber clics de pintado si caen sobre su región en pantalla.
