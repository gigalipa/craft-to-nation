...
- **Previsualización.** Al colocar un puesto (`M`, `H`, `L`, `F`) se ve lo mismo que con un edificio declarado: la plantilla como cajas fantasma en su posición y altura reales (verdes si la colocación es válida, con puertas y ventanas destacadas; rojas si no), las celdas reservadas de despeje de puertas y ventanas (rojas si están bloqueadas) y el área de nivelación de la huella y del frente (cavar, rellenar o ya a nivel). Clic y vista previa usan la misma evaluación (`CamaraCenital._evaluar_puesto()` y `_mensaje_rechazo_puesto()`), así que la vista previa no puede aceptar lo que el clic rechaza. En la pesca el fantasma usa el giro efectivo (el que deja el edificio del lado de tierra). Limitación conocida: en la pesca, la región de nivelación de la cubierta sobre agua se marca como «rellenar» aunque en realidad se colocan pilotes (o se deja el agua abierta).
