...
- **Colocación.** Se rechaza (con mensaje, antes de tocar el mundo) si las celdas de la plantilla no están libres (se comprueban hasta `altura de la plantilla + NiveladorTerreno.LIMITE_PENDIENTE` sobre la superficie), o si el suelo frente a la puerta no está a la altura del nivel objetivo ni 1 bloque más abajo (desde un suelo 1 bloque más alto el dintel de pared impide entrar), es agua o cae dentro de otro puesto. En la pesca, si el extremo de agua detectado no coincide con el de la plantilla girada, se gira 180° para que el edificio caiga del lado de tierra.
...
- `RADIO_SERVICIO` = 2.
- Las plantillas actuales son provisionales (una cabaña por tipo). Sus interiores son pequeños (maderero: 1 celda libre; caza: 3; mina: 8), así que con muchos trabajadores el resto espera fuera; los interiores que diseñe el usuario en SketchUp darán más sitio.
