...
- **`Ciudad.gd`.** `almacen` pasa de 3 a 8 recursos (los cinco nuevos empiezan en 0); `reasignar_tipo(de, a)` mueve un habitante entre tipos sin desajustar `demografia`; `Recurso.tasa_neta` se mide entre cierres de tick consecutivos para incluir lo que entregan los acarreadores entre ticks, y `Recurso.tasa_neta_promedio` es la media de las últimas `VENTANA_TASA_PROMEDIO` = 10 horas (un acarreador tarda ~8–10 h por viaje redondo, así que menos horas seguiría oscilando); `recurso_critico()` usa el promedio.
...
- **`CamaraCenital.gd`.** Al colocar un puesto llama a `Economia.registrar_puesto` con las tasas de su ficha; un clic izquierdo sobre la huella de un puesto, sin ninguna herramienta activa (zonificación `Z`, puestos o blueprint), abre el panel, y cualquier otro clic lo cierra. `Player.gd` llama a `Economia.quitar_puesto` al deconstruir por completo un puesto.
- **`HUD.gd`.** La lista permanente de control de recursos muestra los 8 recursos (cantidad, límite y tasa neta promedio de las últimas 10 h, en `/h`) y reemplaza las dos líneas separadas de comida y "recurso crítico"; aloja el panel del puesto.
...
| Caza | 17 | Decisión del usuario, 2026-09-21 (el primer balance del día dejó 10 y el segundo 14; reemplaza el venado 15 del Excel, que sigue en la hoja `Relacion`); `TASA_BASE_CAZA_POR_CIUDADANO` |
| Frutos | 10 | Partía de la columna "Árbol (obj)" del Excel (Comida 5, interpretada como frutos); cambiado a 8 y luego a 10 por decisión del usuario, 2026-09-21; `TASA_BASE_FRUTOS_POR_CIUDADANO` |
| Pesca | 17 | Decisión del usuario, 2026-09-21 (12 en el primer balance del día; sube para compensar el descuento por profundidad de los peces); `TASA_BASE_PESCA_POR_CIUDADANO` |
...
| Peso de los peces por profundidad | ×0,5 (somera) a ×1,5 (profunda) | Decisión del usuario, 2026-09-21; `Recoleccion.PECES_FACTOR_SOMERO` = 0,5: cada celda pesa `0,5 + (1 − algas)` |
| Agua de referencia (pesca) | 450 celdas | Decisión del usuario, 2026-09-21; `Recoleccion.AGUA_REFERENCIA` (mediana del mundo actual dentro del radio 25) |
| Escala de agua mínima / máxima | 0,4 / 1,5 | Decisión del usuario, 2026-09-21; `Recoleccion.ESCALA_AGUA_MIN` / `ESCALA_AGUA_MAX` |
