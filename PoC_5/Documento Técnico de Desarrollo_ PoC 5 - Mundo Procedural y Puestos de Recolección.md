...
- `detectar_recursos(centro)`: cuenta bloques `"piedra"`/`"hierro"` reales del `GridMap` dentro de una **semiesfera** de radio `RADIO_AREA_MINA` centrada en `centro` (`Vector3.length()` real contra el radio, no una caja alrededor del centro) — la zona de influencia de la mina.
