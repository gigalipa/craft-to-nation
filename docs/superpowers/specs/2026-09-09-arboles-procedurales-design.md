...
4. Devuelve un `Dictionary` (`Vector3i` → `String`, `"madera"` o `"follaje"`) con todos los offsets relativos a la base del árbol (`Vector3i(0,0,0)` es la esquina de la capa de tronco más baja).
...
La **salud** del árbol es el número total de offsets tipo `"madera"` generados (`altura_tronco * lado_tronco²` — varía con `altura_tronco` y `lado_tronco` juntos, un tronco ancho y bajo puede tener tanta madera como uno alto y delgado). El follaje es cosmético, no rinde recurso ni cuenta hacia la salud.
...
6. Traduce cada offset relativo a una celda absoluta (`base + offset`), coloca el bloque real (`colocar_bloque`) con el tipo correspondiente (`"madera"` o `"follaje"`), y registra el árbol completo (`arboles.registrar(celdas_mundiales, altura_tronco)`).
...
Dos bloques placeholder nuevos en la `MeshLibrary` (`BlockLibrarySource.tscn` → regenerar `BlockLibrary.res`), mismo patrón que `tierra`/`piedra`/`hierro`/`agua`: `"madera"` (color plano marrón) y `"follaje"` (color plano verde), sin transparencia ni textura real.
...
- Todo offset tipo `"madera"` tiene `y` en `[0, altura_tronco - 1]`, `dx` en `[0, lado_tronco - 1]` y `dz` en `[0, lado_tronco - 1]`; no hay offsets `"madera"` fuera de ese rango de altura ni de ese cuadrado horizontal.
- El follaje generado nunca pisa una celda de tronco (ninguna celda es a la vez `"madera"` y `"follaje"`).
- La salud devuelta al registrar coincide exactamente con la cantidad de offsets tipo `"madera"` en la forma generada (no con `altura_tronco` sola, ya que ahora depende también de `lado_tronco`).
...
## 4. Correcciones tras confirmar visualmente el bosque (feedback jugando en vivo)

Con el bosque ya visible y espaciado correctamente, el usuario encontró dos problemas de integración con sistemas existentes que no se habían probado hasta entonces:

1. **`VoxelWorld.altura_en()` contaba los bloques de árbol como "el suelo".** Esa función (usada por `ZonaOverlay` para pintar la zona de influencia, y por `CamaraCenital` para ubicar la huella fantasma de nivelación y el disco de previsualización de minas) busca la celda sólida más alta de una columna sin distinguir tipos — así que el overlay de zona y la colocación de minas terminaban aterrizando sobre la copa de un árbol en vez del terreno real debajo. Corregido agregando `VoxelWorld.TIPOS_ARBOL := ["madera", "follaje"]` y haciendo que `altura_en()` salte esas celdas en su búsqueda (sigue bajando hasta encontrar la primera celda sólida que NO sea de árbol). Como todos los consumidores (`ZonaOverlay`, `CamaraCenital`, `Main.gd`, `NiveladorTerreno`, `Recoleccion`) llaman a esta única función, la corrección los arregla a todos a la vez.
2. **`Recoleccion.detectar_recursos()` (la ficha de una mina) contaba bloques de árbol como recurso mineral.** Los bloques `"madera"`/`"follaje"` que caen dentro de la semiesfera de acción de una mina no son del dominio de las minas — pertenecen a un futuro puesto maderero. Corregido agregando `Recoleccion.TIPOS_NO_MINERALES := ["madera", "follaje"]` y excluyéndolos del conteo.

**Renombrado del tipo de bloque:** el tronco pasó de tipo `"tronco"` a tipo `"madera"` en todo el código, las pruebas, la `MeshLibrary` (`BlockLibrarySource.tscn`/`BlockLibrary.res`) y esta spec — `"madera"` es el nombre del recurso real que representa (lo que un futuro puesto maderero recolectará), mientras que "tronco" seguía usándose como palabra para describir la forma/anatomía del árbol (variables como `lado_tronco`, `altura_tronco` no cambiaron, describen geometría, no el recurso).

**Regla de diseño aclarada para un futuro sistema de emplazamiento de edificios (documentada aquí, NO implementada — el juego todavía no tiene un mecanismo de "colocar un edificio completo por huella", solo colocación de bloques uno por uno):** un puesto periférico (p. ej. un puesto maderero) podrá emplazarse dentro de un bosque siempre que su huella no choque con ningún bloque `"madera"`. Si la huella del edificio choca con bloques `"follaje"`, esos bloques de follaje se eliminan automáticamente al construir (el follaje no es un recurso, es solo cosmético). Si algún bloque estructural del edificio chocara con un bloque `"madera"`, la colocación se rechaza con una advertencia del tipo "no se puede colocar una estructura en el mismo lugar que un recurso" — mismo principio que ya rechaza colocar sobre cualquier otro recurso (mineral, etc.), solo que "madera" es el primer caso donde ese recurso convive espacialmente con vegetación no-recurso (follaje) en el mismo objeto.

Verificación de esta ronda: se agregó `TEST 15` a `BlueprintValidatorTest.gd` (`altura_en()` ignora bloques de árbol) y `TEST 5` a `RecoleccionTest.gd` (`detectar_recursos()` ignora `"madera"`/`"follaje"`) — `Test.tscn` ahora tiene 15 pruebas, `RecoleccionTest.tscn` tiene 5. `GeneradorArbolTest.tscn` (10 pruebas) y `GeneradorMundoTest.tscn` (15 pruebas) se re-verificaron sin cambios de comportamiento. `Main.tscn` cargó el mundo real repetidas veces sin errores nuevos.
