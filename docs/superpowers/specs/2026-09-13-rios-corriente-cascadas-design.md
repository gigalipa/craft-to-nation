...
1. Recolectar todas las columnas `(x, z)` con `altura_en(x, z) >= ALTURA_MAXIMA - MARGEN_NACIENTE_RIO` (constante nueva, valor `6` — calibrado jugando en vivo: con `3` solo calificaban las cumbres más altas del mundo, casi sin ríos visibles; con `6`, altura `>= 9`, también nacen ríos de colinas medias) como candidatas brutas a naciente.
