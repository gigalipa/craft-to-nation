...
1. Recolectar todas las columnas `(x, z)` cuya altura cae dentro de la banda `[ALTURA_MAXIMA - MARGEN_NACIENTE_RIO[0], ALTURA_MAXIMA - MARGEN_NACIENTE_RIO[1]]` (constante nueva, `Array[int]` de dos valores `[margen_inferior, margen_superior]`, valor inicial `[5, 1]`) como candidatas brutas a naciente. Antes era un único umbral inferior sin techo (cualquier altura por encima calificaba) — corregido tras jugar en vivo con `AMPLITUD_DETALLE_RELIEVE` alto (ver Sección 1b): el ruido de detalle es más fuerte precisamente cerca de las cumbres, así que la franja más alta del mundo queda con relieve local caótico y el descenso por gradiente (Sección 2) se atasca ahí de inmediato. El margen superior excluye esa franja más caótica, dejando nacer los ríos desde el "hombro" más estable de la montaña.
2. **Una candidata por región elevada, no una por celda** (decisión tomada jugando en vivo: elegir cualquier celda que cruzara el umbral por igual dejaba nacer ríos en el borde de una elevación, no cerca de su cumbre). `_agrupar_regiones_elevadas()` (función pura, sin consultar altura) agrupa las candidatas brutas en regiones conectadas por 4-vecindad — cada región es una montaña/colina separada de las demás por terreno fuera de la banda. Para cada región, `_representante_de_region()` calcula la altura máxima de sus celdas, identifica el subconjunto que comparte esa altura máxima (la cumbre, no toda la ladera), y elige la celda de esa cumbre más cercana al centro geométrico del propio subconjunto — no el centroide de toda la región, y no necesariamente el centroide exacto si este no coincide con ninguna celda real. El resultado (una celda por región) es la lista final de candidatas al sorteo.
3. Con un `RandomNumberGenerator` sembrado (`.seed = semilla + 5`, siguiente hueco libre en la convención `semilla + N` que ya usan `_ruido_mineral`/`_ruido_fauna`/`_ruido_frutal`/`_ruido_arbol`), sortear candidatas sin repetición hasta reunir `NUM_RIOS` (constante nueva, valor `6`) nacientes reales O agotar la lista de candidatas — no exactamente `NUM_RIOS` sorteos. Corrección tras jugar en vivo: con "exactamente `NUM_RIOS` sorteos, sin reintentar", en un mundo real la mayoría de las regiones elevadas son cuencas cerradas que nunca llegan al mar (ver Sección 2b/2, regla de descarte); bastaba que el sorteo cayera en varias de esas para terminar con CERO ríos visibles en todo el mapa. Por cada candidata sorteada se traza de inmediato su cauce crudo (Sección 2); si ese cauce crudo no llega a desembocar en agua, se descarta sin ocupar uno de los `NUM_RIOS` cupos y se sortea la siguiente candidata. Después de cada sorteo (tenga éxito o no), se descartan del resto del sorteo todas las candidatas a menos de `MIN_DISTANCIA_NACIENTES` (constante nueva, valor `10`, distancia en línea recta) de la naciente recién sorteada — protección adicional para regiones muy próximas entre sí. Este filtro no consume el `RandomNumberGenerator`, así que no afecta el determinismo de los sorteos siguientes. Si las candidatas se agotan antes de reunir `NUM_RIOS` nacientes exitosas, se generan menos de `NUM_RIOS` ríos — no es un error.
4. El mismo RNG (mismo objeto, mismo orden de llamadas — determinismo por semilla) elige, para cada candidata sorteada (tenga éxito o no), su `ancho` con `rng.randi_range(ANCHO_MINIMO_RIO, ANCHO_MAXIMO_RIO)` (constantes nuevas, `2` y `6`) — el sorteo de ancho se consume siempre, antes de saber si la candidata llega a agua, para mantener el mismo orden de llamadas al RNG en cualquier reproducción de la selección (ver pruebas).

### 1b. Altura continua y ruido de detalle en picos/valles (corrección tras prueba en vivo)

`altura_en(x, z)` redondea la altura a solo 16 niveles enteros en todo el mundo — suficiente para el terreno normal, pero insuficiente para el descenso por gradiente de `_trazar_rio()` (Sección 2): comparar alturas enteras hacía que el trazado detectara "mesetas" falsas (artefactos de cuantización) y terminara casi de inmediato. Se agregó `_altura_flotante(x, z) -> float` (altura continua, sin redondear) usada en todo el descenso; `altura_en()` pasa a ser un envoltorio que redondea/recorta el resultado de `_altura_flotante()` — el terreno visible no cambia.

Al probar con valores experimentales extremos de relieve (`EXPONENTE_RELIEVE` bajo), se detectó que la curva de redistribución `sign(x)*|x|^exponente` comprime cualquier valor de ruido por encima de cierto umbral hacia el extremo `±1` — con exponentes bajos esto produce mesetas anchas en vez de picos puntiagudos (confirmado matemáticamente, no es un bug del trazado de ríos). Corrección: `_ruido_detalle` (nuevo `FastNoiseLite`, `seed = semilla + 6`, frecuencia `0.1`) se suma a `_altura_flotante()` escalado por `AMPLITUD_DETALLE_RELIEVE` (constante nueva, valor `3.0`) y por `abs(valor_redistribuido)` (cercano a `1` en los extremos — cumbres/fondos, cercano a `0` en laderas intermedias) — rompe visualmente las mesetas cerca de picos y valles sin alterar el resto del relieve.
...
### 4b. Caída visual real en cascadas (corrección tras prueba en vivo)

Hasta aquí, cada columna de una franja de río se talla usando solo su propia altura local (`h = generador.altura_en(x, z)`) — una celda marcada `es_cascada_en()` no cambiaba este cálculo, así que el agua de la celda superior quedaba como una laguna flotante sobre un acantilado seco: el salto de altura existía como dato (`es_cascada_en()`, Sección 5) pero no como geometría tallada. Confirmado jugando en vivo ("las cascadas no caen").

Corrección en `VoxelWorld._generar_terreno()`: para cualquier columna `(x, z)` donde `generador.es_cascada_en(x, z)` es verdadero, el inicio del relleno de agua ya no es solo `h`, sino que se extiende hacia abajo hasta `min(h, generador.altura_en(siguiente.x, siguiente.y) + 1)`, donde `siguiente = (x, z) + generador.direccion_flujo_en(x, z)` (la celda del cauce a la que fluye esta franja). Esto talla una lámina de agua continua a lo largo de toda la cara del salto, en vez de dejar el hueco entre la altura de llegada y la de salida. El resto de la talla (profundidad de franja, subsuelo debajo del agua) no cambia.
