...
1. Recolectar todas las columnas `(x, z)` con `altura_en(x, z) >= ALTURA_MAXIMA - MARGEN_NACIENTE_RIO` (constante nueva, valor `10` — calibrado jugando en vivo: con `3` solo calificaban las cumbres más altas del mundo, casi sin ríos visibles; con `6`, altura `>= 9`, también nacían ríos de colinas medias; subido después a `10`, altura `>= 5`, para ampliar aún más el rango de colinas candidatas) como candidatas brutas a naciente.
2. **Una candidata por región elevada, no una por celda** (decisión tomada jugando en vivo: elegir cualquier celda que cruzara el umbral por igual dejaba nacer ríos en el borde de una elevación, no cerca de su cumbre). `_agrupar_regiones_elevadas()` (función pura, sin consultar altura) agrupa las candidatas brutas en regiones conectadas por 4-vecindad — cada región es una montaña/colina separada de las demás por terreno bajo el umbral. Para cada región, `_representante_de_region()` calcula la altura máxima de sus celdas, identifica el subconjunto que comparte esa altura máxima (la cumbre, no toda la ladera), y elige la celda de esa cumbre más cercana al centro geométrico del propio subconjunto — no el centroide de toda la región, y no necesariamente el centroide exacto si este no coincide con ninguna celda real. El resultado (una celda por región) es la lista final de candidatas al sorteo.
3. Con un `RandomNumberGenerator` sembrado (`.seed = semilla + 5`, siguiente hueco libre en la convención `semilla + N` que ya usan `_ruido_mineral`/`_ruido_fauna`/`_ruido_frutal`/`_ruido_arbol`), elegir hasta `NUM_RIOS` (constante nueva, valor `6`) de esas candidatas (una por región) al azar sin repetición como nacientes reales. Después de cada elección, se descartan del resto del sorteo todas las candidatas a menos de `MIN_DISTANCIA_NACIENTES` (constante nueva, valor `20`, distancia en línea recta) de la naciente recién elegida — protección adicional para regiones muy próximas entre sí. Este filtro no consume el `RandomNumberGenerator`, así que no afecta el determinismo de los sorteos siguientes. Si hay menos regiones que `NUM_RIOS`, o se agotan por el filtro de distancia, se generan menos de `NUM_RIOS` ríos — no es un error.
4. El mismo RNG (mismo objeto, mismo orden de llamadas — determinismo por semilla) elige a continuación, para cada naciente, su `ancho` con `rng.randi_range(ANCHO_MINIMO_RIO, ANCHO_MAXIMO_RIO)` (constantes nuevas, `2` y `6`).
