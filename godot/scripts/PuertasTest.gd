...
func ejecutar_pruebas() -> void:
	...
	var centro_x := Vector3(b_x) + Vector3(0.5, 1.0, 0.5)
	assert(cuerpo.position.is_equal_approx(centro_x + Vector3(0.45, 0, 0)), "pared por X: abierta queda pegada a la jamba (dentro de la celda), no en el centro")
	assert(p.alternar(b_x))
	...
	assert(is_equal_approx(p.cuerpo_de(b_z).rotation.y, PI), "pared por Z: abrir suma 90° a los 90° de la orientación")
